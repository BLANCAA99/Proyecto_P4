...
namespace WebApiProyectoFinal
{
    public class Auth
    {
        ...
        public static void login(int codUsuario, string password,int codCentro)
        {
            ...
            var usuario = db.Usuarios.FirstOrDefault(x=>x.Codigo ==codUsuario);
            var centros = db.CentrosAsistencias.FirstOrDefault(x => x.Id_Asistencia == codCentro);
            const string codRolDigitador = "3";
           ...
           // var getAsistenciaByIdQueryModel = new GetAsistenciaByIdQuery();
            ...
            newCurrentAuth.centroAsistencia = centros.Descripcion;
